...
/**
 * СПИСОК ЧАТОВ СВИДАНИЙ — layout как WhatsApp, цвета Aru
 */
...
<style>
    ...
    .chat-menu-item.is-warn {
        color: #b45309;
    }

    .chat-menu-item.is-warn i {
        color: #b45309;
    }
...
<div class="chats-page-container">
    <div class="chats-header">
        <div class="chats-header-content">
            <a href="<?= BASE_URL ?>dates" class="btn-back-modern" title="Назад" aria-label="Назад">
                ...
                <div class="chat-card-modern" id="chat-wrapper-<?= $date['id'] ?>">
                    ...
                    <div class="chat-actions-modern">
                        <button type="button"
                            ...
                            aria-label="Действия"
                            aria-expanded="false"
                            onclick="toggleChatMenu(<?= (int)$date['id'] ?>, event)">
                            <i class="bi bi-three-dots-vertical"></i>
                        </button>
                        <div class="chat-menu-dropdown" id="chat-menu-<?= $date['id'] ?>">
                            <button type="button"
                                class="chat-menu-item is-warn"
                                onclick="blockAndDeleteDateChat(<?= (int)$date['id'] ?>, <?= (int)$chatUserId ?>, event)">
                                <i class="bi bi-lock-fill"></i>
                                <span>Заблокировать</span>
                            </button>
                            <button type="button"
                                class="chat-menu-item is-danger"
                                onclick="deleteDateChat(<?= (int)$date['id'] ?>, event)">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        window.toggleChatMenu = function(dateId, e) {
            if (e) {
                e.preventDefault();
                e.stopPropagation();
            }

            const menu = document.getElementById('chat-menu-' + dateId);
            ...
            fetch(BASE_URL + 'messages/blockAndDeleteDateChat', {
                ...
                .then(data => {
                    if (data && data.success === true) {
                        ...
                        alert(data?.error || data?.message || 'Не удалось заблокировать пользователя и удалить чат.');
                    }
                })
                .catch(error => {
                    wrapper.style.opacity = '1';
