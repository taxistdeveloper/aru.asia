...
/**
 * СТРАНИЦА СПИСКА ЧАТОВ МЕРОПРИЯТИЙ
 */
...
<div class="chats-page-container">
    <div class="chats-header">
        <div class="chats-header-content">
            <a href="<?= BASE_URL ?>events" class="btn-back-modern">
                    ...
                    <a href="<?= BASE_URL ?>messages/event?event_id=<?= $event['id'] ?>" class="chat-card-link">
                        <div class="chat-avatar-modern">
                            <?php if (!empty($event['photo'])): ?>
                                <img src="<?= BASE_URL . UPLOAD_DIR . 'photos/' . $event['photo'] ?>" alt="<?= Helper::escape($event['title']) ?>">
                            <?php else: ?>
                                <div class="chat-avatar-placeholder-modern">
                                    <i class="bi bi-calendar-event-fill"></i>
                                </div>
                            <?php endif; ?>
                            <?php if (!empty($event['unread_count']) && $event['unread_count'] > 0): ?>
                                <span class="chat-unread-badge-modern">
                                    <?= $event['unread_count'] > 99 ? '99+' : $event['unread_count'] ?>
                                </span>
                            <?php endif; ?>
                        </div>
                        <div class="chat-info-modern">
                            <div class="chat-header-modern">
                                <?php if (!empty($event['title'])): ?>
                                    <h3 class="chat-title-modern"><?= Helper::escape($event['title']) ?></h3>
                                <?php else: ?>
                                ...
                                <span class="chat-time-modern">
                                    ...
                                    $dateText = date('d.m.Y', $eventTime);
                                    $timeText = date('H:i', $eventTime);
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        window.deleteEventChat = function(eventId, e) {
            ...
            console.log('Отправка запроса на удаление чата:', eventId);
            console.log('URL:', BASE_URL + 'messages/deleteEventChat');
...
<style>
    ...
    .chat-card-modern:hover .chat-delete-btn-modern {
        opacity: 1;
        pointer-events: auto;
    }
