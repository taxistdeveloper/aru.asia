...
class AdminController
{
    ...
    public function index()
    {
        ...
        try {
            ...
            $stats['users_today'] = $safeQuery("SELECT COUNT(*) FROM users WHERE DATE(created_at) = CURDATE()", 0);
            $stats['events_today'] = $safeQuery("SELECT COUNT(*) FROM events WHERE DATE(created_at) = CURDATE()", 0);
            $stats['dates_today'] = $safeQuery("SELECT COUNT(*) FROM dates WHERE DATE(created_at) = CURDATE()", 0);

            // Посещения сайта (лендинг) за сегодня
            $stats['visits_today'] = $safeQuery("SELECT visits_total FROM daily_visits WHERE visit_date = CURDATE()", 0);
            $stats['unique_today'] = $safeQuery("SELECT unique_total FROM daily_visits WHERE visit_date = CURDATE()", 0);

            // Статистика за неделю
            $stats['users_week'] = $safeQuery("SELECT COUNT(*) FROM users WHERE created_at >= DATE_SUB(NOW(), INTERVAL 7 DAY)", 0);
            $stats['events_week'] = $safeQuery("SELECT COUNT(*) FROM events WHERE created_at >= DATE_SUB(NOW(), INTERVAL 7 DAY)", 0);
        ...
        try {
            ...
            $safeQuery = function ($query, $default = 0) use ($db) {
                try {
                    ...
                    error_log("AdminController::stats - Query error: " . $e->getMessage());
                    return $default;
                }
            };
            ...
            $safeFetchAll = function ($query, $default = []) use ($db) {
                try {
                    ...
                    error_log("AdminController::stats - FetchAll error: " . $e->getMessage());
                    return $default;
                }
            };
